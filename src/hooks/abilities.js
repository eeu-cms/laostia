...
async function defineAbilitiesFor(ctx, options) {
  ...
  async function grantAccess(accessList, withAuthentication = false) {
    for (let i = 0; i < accessList.length; i++) {
      ...
      if (access.method && !Array.isArray(access.method)) access.method = [access.method];

      if (!access.method || !access.method.length || !access.service) {
        throw new GeneralError('Error Abilities', { index: i, message: 'Servicio o metodo invalido' });
      }

      if (options.serviceName === access.service && access.method.includes(options.action)) {
        ...
        if (['get', 'find', 'update', 'patch', 'remove'].includes(options.action)) {
          if (query) {
  ...
  const userAccessServices = [
    ...
    { method: ['get', 'find', 'update', 'patch'], service: 'orders', query: Q.isOwner },
